...
try {
    ...
    writer.writeLine('//THIS CODE IS AUTOGENERATED - DO NOT MODIFY MANUALLY//');

    selectorPaths.forEach((absolutePath, index) => {
        const { dir, name, ext } = path.parse(absolutePath);

        // TODO: write a path sanitizer
        const relativePath = path.relative(from,dir);

        writer.writeLine(`export * from '${relativePath}/${name}';`);
    });
    ...
    const msgBox = boxen( chalk.red.bold('SOMETHING WENT WRONG 🤕', e.message), boxenOptions );
    console.log(msgBox);
}